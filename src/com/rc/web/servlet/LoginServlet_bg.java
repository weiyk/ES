 ...
 * @version ver 1.0
 */
public class LoginServlet_bg extends BaseServlet {
	...
	private static final Log log = LogFactory.getLog(LoginServlet_bg.class);
	/**
	 * 不允许直接通过录入URL访问该servlet
	 * */
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		this.forwarToTarget("/bglogin.jsp",request,response);
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			sql.append(" WHERE  YH.ZHUANGTAI='1' AND loginId ='").append(loginName).append("'");
			log.debug("用户sql:"+sql);
			int i = 0;
			...
			if(list.size()>0){
				...
				throw new Exception("该用户没有操作权限，登录失败");
			}
			...
			//log.error("用户登录失败", e);
			this.forwarToTarget("/bglogin.jsp?errInfo=" + e.getMessage(),request, response);
		} finally {
			try{
				rs.close();
