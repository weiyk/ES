...
<head>
	...
	<script type="text/javascript" src="<%=style%>/js/base.js"></script>
	 <html:dialog id="GNDH_GongGaoSTUXQ" state=""/>
</head>
<%
...
<div class="wrap clearfix">
	...
	<div class="fl mt-20 w-710">
         <!-- 所在位置 -->
         <div class="td-zt">
             您所在的位置：<a href="<%=path%>/servlet/jspdispatchservlet?_qam_dialog=GNDH_XueShengKJ" >首页</a>&gt;
             	  <a href="<%=path%>/servlet/jspdispatchservlet?_qam_dialog=GNDH_TongZhiSTUGG">学生动态</a>&gt;
             	通知详情
         </div>
    ...
    <div class="next-more">
	    	<ul> 
		    	<li>&gt;上一条：
		    	  	<c:if test="${beforId != ''}">
		    			<a href="<%=path%>/servlet/jspdispatchservlet?_qam_dialog=GNDH_GongGaoSTUXQ&cs_BianHao=<%=beforId%>">
		    				<%=beforTitle%>
		    			</a>
		    	   	</c:if>
		    	   	<c:if test="${beforId == ''}">
		    	...
		    	<li>&gt;下一条：
		    		<c:if test="${afterId != ''}">
		    			<a href="<%=path%>/servlet/jspdispatchservlet?_qam_dialog=GNDH_GongGaoSTUXQ&cs_BianHao=<%=afterId%>" >
		    				<%=afterTitle%>
		    			</a>
		    		</c:if>
		    		<c:if test="${afterId ==''}">
